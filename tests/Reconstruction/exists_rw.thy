...
ML_val \<open>
  val premise = mk @{prop "A \<Longrightarrow> B (\<exists>x. (Q :: 'b \<Rightarrow> 'c \<Rightarrow> bool) x c) = B' \<Longrightarrow> False"}
  val expected = mk @{prop "A \<Longrightarrow> B ((Q :: 'b \<Rightarrow> 'c \<Rightarrow> bool) (SOME x. Q x c) c) = B' \<Longrightarrow> False"}
  val predicate = @{cterm "\<lambda>x. (Q :: 'b \<Rightarrow> 'c \<Rightarrow> bool) x c"}
  val subterm = ([1], JLit.Left, 1)
  val conclusion =
    Jeha_Proof.reconstruct_exists_rw
      @{context}
