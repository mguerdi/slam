...
ML_val \<open>
  ...
  val bad = Slam.simp_false_elim @{context} c (lpos, cpos);
  val lit = JClause.lit_at cpos c;
  val (s, b) = lit |> JLit.dest_pred ||> Slam.ml_bool_of;
  val c' = JClause.delete_lit_at cpos c;
  val bad_direct = Slam.outer_clausify b s c;
\<close>

ML_val \<open>
  val good = JClause.of_term (@{term_schem "?x b = True \<or> (?x a = True) = False"}, 1);
  val simplified = Slam.simp_false_elim @{context} good (JLit.Left, 0);
