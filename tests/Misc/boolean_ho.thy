...
ML_val \<open>
  val c = JClause.of_term @{context} (@{term_schem "(\<forall> x. ?y x = (p x \<and> q x)) = False"}, 0);
  val forall_subterm_position = ([], JLit.Left, 0);
  val forall_subterm = JClause.subterm_at_full_pos c forall_subterm_position;
  val [conclusion] = Slam.infer_forall_rw @{context} c forall_subterm_position; 
  ...
  (* val () = writeln (Slam_Common.pretty_term @{context} expected);
  val () = writeln (Slam_Common.pretty_term @{context} instead_of_forall); *)
