...
proof (induct t)
  ...
  proof -
    ...
    have "\<not> ?p\<^sub>1 (Suc 0)" by (slam reflect.simps(1) n_leaves.simps(1))
    hence "\<not> ?p\<^sub>1 (n_leaves (Lf::'a bt))" by (slam n_leaves.simps(1))
    thus "n_leaves (reflect (Lf::'a bt)) = n_leaves (Lf::'a bt)" by slam
  qed
next
  case (Br a t1 t2) thus ?case
    by (slam n_leaves.simps(2) add.commute reflect.simps(2))
qed

lemma n_nodes_reflect: "n_nodes (reflect t) = n_nodes t"
proof (induct t)
  case Lf thus ?case by (slam reflect.simps(1))
next
  case (Br a t1 t2) thus ?case
    by (slam add.commute n_nodes.simps(2) reflect.simps(2))
qed

lemma depth_reflect: "depth (reflect t) = depth t"
apply (induct t)
 apply (slam depth.simps(1) reflect.simps(1))
by (slam depth.simps(2) max.commute reflect.simps(2))
...
apply (induct t)
 apply (slam n_leaves.simps(1) n_nodes.simps(1))
by auto

lemma reflect_reflect_ident: "reflect (reflect t) = t"
apply (induct t)
 apply (slam reflect.simps(1))
proof -
...
apply (rule ext)
apply (induct_tac y)
 apply (slam bt_map.simps(1))
by (slam bt_map.simps(2))

lemma bt_map_append: "bt_map f (append t u) = append (bt_map f t) (bt_map f u)"
(* apply (induct t)
 apply (slam append.simps(1) bt_map.simps(1))
proof -
  (* FIXME: timeout with 500 *)
  note [[slam_max_number_of_steps = 1000]]
  (* have ?thesis by (slam append.simps(2) bt_map.simps(2)) *)
qed *)
sorry
...
(* by (slam bt_map.simps(2) o_eq_dest_lhs) *)

lemma bt_map_reflect: "bt_map f (reflect t) = reflect (bt_map f t)"
apply (induct t)
 apply (slam bt_map.simps(1) reflect.simps(1))
by (slam bt_map.simps(2) reflect.simps(2))

lemma preorder_bt_map: "preorder (bt_map f t) = map f (preorder t)"
apply (induct t)
 apply (slam bt_map.simps(1) list.map(1) preorder.simps(1))
by simp
...
proof (induct t)
  case Lf thus ?case
  proof -
    have "map f [] = []" by (slam list.map(1))
    hence "map f [] = inorder Lf" by (slam inorder.simps(1))
    hence "inorder (bt_map f Lf) = map f []" by (slam bt_map.simps(1))
    thus "inorder (bt_map f Lf) = map f (inorder Lf)" by (slam inorder.simps(1))
  qed
next
...
apply (induct t)
 apply (slam Nil_is_map_conv bt_map.simps(1) postorder.simps(1))
by simp

lemma depth_bt_map [simp]: "depth (bt_map f t) = depth t"
apply (induct t)
 apply (slam bt_map.simps(1) depth.simps(1))
by simp

lemma n_leaves_bt_map [simp]: "n_leaves (bt_map f t) = n_leaves t"
apply (induct t)
 apply (slam bt_map.simps(1) n_leaves.simps(1))
proof -
  ...
  have "\<And>V U. n_leaves (Br U (bt_map f t1) V) = n_leaves t1 + n_leaves V"
    using A1 by (slam n_leaves.simps(2))
...
apply (induct t)
 apply (slam Nil_is_rev_conv postorder.simps(1) preorder.simps(1)
              reflect.simps(1))
apply simp
done
...
apply (induct t)
 apply (slam Nil_is_rev_conv inorder.simps(1) reflect.simps(1))
by simp
(* Slow:
by (slam append.simps(1) append_eq_append_conv2 inorder.simps(2)
...
apply (induct t)
 apply (slam Nil_is_rev_conv postorder.simps(1) preorder.simps(1)
              reflect.simps(1))
by (slam preorder_reflect reflect_reflect_ident rev_swap)
...
apply (induct t1)
 apply (slam append.simps(1))
by (slam append.simps(2))
...
apply (induct t1)
 apply (slam append.simps(1) depth.simps(1) plus_nat.simps(1))
by simp

lemma n_leaves_append [simp]:
     "n_leaves (append t1 t2) = n_leaves t1 * n_leaves t2"
apply (induct t1)
 apply (slam append.simps(1) n_leaves.simps(1) nat_mult_1 plus_nat.simps(1)
              Suc_eq_plus1)
