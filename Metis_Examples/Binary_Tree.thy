...
apply (induct t)
 apply (jeha Nil_is_rev_conv postorder.simps(1) preorder.simps(1)
              reflect.simps(1))
by (jeha preorder_reflect reflect_reflect_ident rev_swap)
...
apply (induct t1)
 apply (jeha append.simps(1))
by (jeha append.simps(2))
...
apply (induct t1)
 apply (jeha append.simps(1) depth.simps(1) plus_nat.simps(1))
by simp

lemma n_leaves_append [simp]:
     "n_leaves (append t1 t2) = n_leaves t1 * n_leaves t2"
apply (induct t1)
 apply (jeha append.simps(1) n_leaves.simps(1) nat_mult_1 plus_nat.simps(1)
              Suc_eq_plus1)
