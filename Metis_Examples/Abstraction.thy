...
theory Abstraction
  imports "SLAM.slam" "HOL-Library.FuncSet"
begin
...
by (slam nat_1_add_1 trans_less_add2)
...
lemma simpler:
  ...
  using [[slam_max_number_of_steps = 100
...
lemma "(=) = (\<lambda>x y. y = x)"
  using [[unify_search_bound = 15
        ...
        (* by (slam better_ext) (* ext epsilon_characterization) *) *)
  oops
...
proof -
  assume "a \<in> {x. P x}"
  thus "P a" by (slam mem_Collect_eq)
qed

lemma Collect_triv: "a \<in> {x. P x} \<Longrightarrow> P a"
by (slam mem_Collect_eq)

lemma "a \<in> {x. P x --> Q x} \<Longrightarrow> a \<in> {x. P x} \<Longrightarrow> a \<in> {x. Q x}"
by (slam Collect_imp_eq ComplD UnE)

lemma "(a, b) \<in> Sigma A B \<Longrightarrow> a \<in> A \<and> b \<in> B a"
proof -
  assume A1: "(a, b) \<in> Sigma A B"
  hence F1: "b \<in> B a" by (slam mem_Sigma_iff)
  have F2: "a \<in> A" by (slam A1 mem_Sigma_iff)
  have "b \<in> B a" by (slam F1)
  thus "a \<in> A \<and> b \<in> B a" by (slam F2)
qed

lemma Sigma_triv: "(a, b) \<in> Sigma A B \<Longrightarrow> a \<in> A & b \<in> B a"
by (slam SigmaD1 SigmaD2)

lemma "(a, b) \<in> (SIGMA x:A. {y. x = f y}) \<Longrightarrow> a \<in> A \<and> a = f b"
by (slam (full_types, lifting) CollectD SigmaD1 SigmaD2)

lemma "(a, b) \<in> (SIGMA x:A. {y. x = f y}) \<Longrightarrow> a \<in> A \<and> a = f b"
proof -
  assume A1: "(a, b) \<in> (SIGMA x:A. {y. x = f y})"
  hence F1: "a \<in> A" by (slam mem_Sigma_iff)
  have "b \<in> {R. a = f R}" by (slam A1 mem_Sigma_iff)
  hence "a = f b" by (slam (full_types) mem_Collect_eq)
  thus "a \<in> A \<and> a = f b" by (slam F1)
qed

lemma "(cl, f) \<in> CLF \<Longrightarrow> CLF = (SIGMA cl: CL.{f. f \<in> pset cl}) \<Longrightarrow> f \<in> pset cl"
by (slam Collect_mem_eq SigmaD2)

lemma "(cl, f) \<in> CLF \<Longrightarrow> CLF = (SIGMA cl: CL.{f. f \<in> pset cl}) \<Longrightarrow> f \<in> pset cl"
proof -
  assume A1: "(cl, f) \<in> CLF"
  assume A2: "CLF = (SIGMA cl:CL. {f. f \<in> pset cl})"
  have "\<forall>v u. (u, v) \<in> CLF \<longrightarrow> v \<in> {R. R \<in> pset u}" by (slam A2 mem_Sigma_iff)
  hence "\<forall>v u. (u, v) \<in> CLF \<longrightarrow> v \<in> pset u" by (slam mem_Collect_eq)
  thus "f \<in> pset cl" by (slam A1)
qed

lemma
  "(cl, f) \<in> (SIGMA cl: CL. {f. f \<in> pset cl \<rightarrow> pset cl}) \<Longrightarrow>
   f \<in> pset cl \<rightarrow> pset cl"
by (slam (no_types) Collect_mem_eq Sigma_triv)
...
proof -
  ...
  thus "f \<in> pset cl \<rightarrow> pset cl" by (slam mem_Collect_eq)
qed
...
by (slam (no_types) Collect_conj_eq Int_def Sigma_triv inf_idem)
*)
...
proof -
  ...
  hence "f \<in> Id_on cl `` pset cl" by (slam Int_commute Image_Id_on mem_Collect_eq)
  hence "f \<in> cl \<inter> pset cl" by (slam Image_Id_on)
  thus "f \<in> pset cl \<inter> cl" by (slam Int_commute)
qed
...
lemma
  "(cl, f) \<in> CLF \<Longrightarrow>
   ...
   f \<in> pset cl \<inter> cl"
by (slam (lifting) CollectD Sigma_triv subsetD)

lemma
  "(cl, f) \<in> CLF \<Longrightarrow>
   CLF = (SIGMA cl: CL. {f. f \<in> pset cl \<inter> cl}) \<Longrightarrow>
   f \<in> pset cl \<inter> cl"
by (slam (lifting) CollectD Sigma_triv)
