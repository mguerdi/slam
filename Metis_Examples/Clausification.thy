...
lemma
  ...
  shows "lim f \<le> x"
by (slam 1 LIMSEQ_le_const2 fn_le)
...
using [[slam_trace]] by (slam bounded_def subset_eq)
*)
...
using [[slam_trace]] by (slam (full_types))
*)
...
by (slam split_list_last_prop[where P = P] in_set_conv_decomp)
*)
