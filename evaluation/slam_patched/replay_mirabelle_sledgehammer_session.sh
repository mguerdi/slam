...
"$HOME/Isabelle/bin/isabelle" mirabelle -o "threads=1" -t 10 -g 1 -O "${out_dir}" -A "sledgehammer_replay[provers=zipperposition, mini_preplay_inputs=true, proof_methods=\"${meths}\", instantiate=false, preplay_timeout=2, smt_proofs=false]" -X slow "${thy}"
