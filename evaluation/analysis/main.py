...
def parse_file(mirabelle_log, timeout_ms, only_theory, only_session):
    lines = [
        squeezed
        for line in mirabelle_log.strip().split("\n")
    ...
    for line in lines:
        ...
        if command.startswith("("):
            if not command.endswith(")"):
                raise RuntimeError(
                    "Command " + repr(command) + " starts with '(' but doesn't end with ')'"
                )
            command = command[1:-1]
        ...
        if "slam" in command:
            ...
            raise RuntimeError(
                "Line is neither slam nor metis invocation nor mirabelle timeout:\n" + line
            )
    ...
    print(
        "EXCLUDED",
        excluded_not_from_theory_or_session,
        "GOALS NOT FROM SPECIFIED THEORY OR SESSION",
    )
    return calls
...
def goals_with_differing_facts(all_goals, slam_calls, metis_calls):
    ...
    differing_facts_slam_better_and_metis_dubious_goals = set(
        differing_facts_slam_better_and_metis_dubious_goal_list
    )
    return (
        differing_facts_goals,
        differing_facts_slam_better_goals,
        differing_facts_slam_better_and_metis_dubious_goals,
    )
...
def summarize(
    ...
    (
        ...
        differing_facts_slam_better_goals,
        differing_facts_slam_better_and_metis_dubious_goals,
    ) = goals_with_differing_facts(all_goals, slam_calls, metis_calls)
    print("TOTAL GOALS WITH DIFFERING FACTS:", len(differing_facts_goals))
    print(
        "TOTAL GOALS WITH DIFFERING FACTS WHERE SLAM IS BETTER:",
        len(differing_facts_slam_better_goals),
    )
    print(
        "TOTAL GOALS WITH DIFFERING FACTS WHERE SLAM IS BETTER AND METIS DUBIOUS:",
    ...
    def extrapolate(time):
        return np.exp(
            np.sum(
                [fit[i] * np.log(time) ** (polyfit_degree - i) for i in range(polyfit_degree + 1)]
            )
        )

    polyfit_label = " + ".join(
        f"{fit[i]:.2f} x**{polyfit_degree - i}" for i in range(polyfit_degree + 1)
    )
    ...
    metis_better = sum(
        1
        for (metis_time, slam_time) in zip(metis_both_success_times, slam_both_success_times)
        if metis_time < slam_time
    )
    slam_better = sum(
        1
        for (slam_time, metis_time) in zip(slam_both_success_times, metis_both_success_times)
        if slam_time < metis_time
    )
    ...
    if plot_scatter:
        ...
        plt.scatter(
            metis_both_success_times, slam_both_success_times, marker=".", s=14, lw=0, alpha=0.5
        )  # , c=color, alpha=0.5) # , label=label)

        plt.plot(
            [min_time_minus, max_time_plus], [min_time_minus, max_time_plus], color="red", alpha=0.5
        )  # , label="diagonal")
...
def plot_success_calls(metis_calls, slam_calls, label, invocation, scale_metis=False):
    cm = plt.get_cmap("nipy_spectral")
    color = cm(((135 * invocation) % 360) / 360.0)
    ...
    if scale_metis:
        metis_cumulative_problems = [
            i * 1000.0 / metis_successes for i, _ in enumerate(metis_success)
        ]
    else:
        metis_cumulative_problems = [i for i, _ in enumerate(metis_success)]

    plt.plot(
        metis_success_times, metis_cumulative_problems, "+", color=color, label=label + " (metis)"
    )
    ...
    if scale_metis:
        slam_cumulative_problems = [
            i * 1000.0 / metis_successes for i, _ in enumerate(slam_success)
        ]
    else:
        slam_cumulative_problems = [i for i, _ in enumerate(slam_success)]

    plt.plot(
        slam_success_times, slam_cumulative_problems, "+", color=color, label=label + " (slam)"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-pcsm",
        "--plot-cactus-scaled-metis",
        action="store_true",
        help="create FAKE cactus plot scaled to have all metis runs equal",
    )
    ...
    parser.add_argument(
        "-th", "--theory", type=str, help="Only include goals from the given theory."
    )
    parser.add_argument(
    ...
    if (
        ...
        raise RuntimeError(
            "specify at most one of --plot-cactus, --plot-cactus-scaled-metis, --plot-scatter and --plot-hist"
        )

    plot_any = (
        args.plot_cactus or args.plot_cactus_scaled_metis or args.plot_scatter or args.plot_hist
    )
    if plot_any:
        import matplotlib
    ...
    for i, dirname in enumerate(runs_dirs):
        ...
        if os.path.isfile(filepath):
            ...
            session_log_dirs = [
                name
                for name in os.listdir(mirabelle_output_dir)
                if os.path.isdir(mirabelle_output_dir + "/" + name)
            ]
            mirabelle_log = ""
    ...
    if args.plot_cactus_scaled_metis:
        ...
        plt.legend()
        plt.title(
            title_prefix + "FAKE! RUNS HAVE BEEN SCALED SO THAT METIS RUNS HAVE THE SAME HEIGHT!"
        )
