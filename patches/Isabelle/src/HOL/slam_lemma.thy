...
imports Hilbert_Choice Transfer slam_base

begin

ML_file \<open>Tools/Slam/slam_proof_util.ML\<close>
...
structure Slam_Lemma: SLAM_LEMMA =
struct

(* see Slam.bool_rw_non_var_rules *)
...
fun impl_forall_exists_rw_lemma ctxt predicate is_forall_rw (sk, sk_with_args, sk_def) =
  let
    ...
    val th =
      ...
      |> Thm.cterm_of ctxt
      |> Goal.init
      |> Slam_Proof_Util.with_all_vars_fixed ctxt (HEADGOAL (fa_ex_rw_lemma_tac ctxt is_forall_rw))
      |> Seq.hd
      |> Goal.finish ctxt
  in
    th
...
fun neg_ext_lemma ctxt (s, s') (sk, sk_with_args, sk_def) =
  let
    ...
    val th =
      ...
      |> Thm.cterm_of ctxt
      |> Goal.init
      |> Slam_Proof_Util.with_all_vars_fixed ctxt (HEADGOAL (neg_ext_lemma_tac ctxt))
      |> Seq.hd
      |> Goal.finish ctxt
  in
    th
